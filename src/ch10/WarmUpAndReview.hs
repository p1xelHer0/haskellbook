...
--    although the stop-vowel-stop pattern is common enough that many of them
--    will.
stopVowelStop =
  [ (x, y, z)
  | x <- stops
...
--    modify the function to make tuples representing possible
...
-- 2. What does the following mystery function do? What is its type? Try to
--    get a good sense of what it does before you test it in the REPL
--    to verify it.
-- 2. it calculates the average length of each word int the argument
seekritFunc x = div (sum (map length (words x))) (length (words x))

-- 3. We’d really like the answer to be more precise. Can you rewrite that
--    using fractional division?
seektritFuncFrac x =
